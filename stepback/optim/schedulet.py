"""
Adapted from: https://github.com/facebookresearch/schedule_free/blob/main/schedulefree/sgd_schedulefree.py
...
class SGDSchedulet(torch.optim.Optimizer):
    r"""
    Schedule-Free SGD with theoretically inspired weights (the c_t's)
...
class AdamWSchedulet(torch.optim.Optimizer):
    r"""
    Schedule-Free AdamW with theoretically inspired weights (the c_t's)
    As the name suggests, no scheduler is needed with this optimizer. 
    To add warmup, rather than using a learning rate schedule you can just
    set the warmup_steps parameter.
